...
namespace statlog {
    ...
    public:
        explicit sync_logger_t(std::string_view name, sink_pointer<Sinks>... sinks) : logger<sync_logger_t>(name), _sinks(std::forward<sink_pointer<Sinks>>(sinks)...) {}
    ...
    private:
        std::tuple<sink_pointer<Sinks>...> _sinks;
    };
